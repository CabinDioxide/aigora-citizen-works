...
const UI = {
  ...
  ov_sub: ['真实地图上：咽喉点状态环（IMF PortWatch 近七日通过艘数对基线：停摆红、收窄橙、在用绿）、战略站点按影像判读着色、Stack Atlas 事件标记；三个战区的供应链依赖关系是另一层地铁图，点地图左上角的按钮才显示。右侧是三个战区今天的读数，下面是今天的警报',
    'On a real map: chokepoint status rings (IMF PortWatch 7-day transits against baseline: red stopped, amber narrowed, green in use), strategic sites colored by imagery reading, Stack Atlas event markers; the supply-chain dependencies of the three theaters are a separate metro-map layer, shown only when you press the button at the top left of the map. Right: today\'s readings for the three theaters; below: today\'s alerts'],
  ...
  metro_sub: ['真实地图，默认显示岸站船位（箭头是航向）、卫星影像显著变化（红＝受损迹象，橙＝局部或整幅变化，蓝＝热异常）、异常航迹、关键节点编号。{n} 条链的供应链依赖关系是叠在最上面的地铁图，点地图左上角的「供应链依赖」才显示：线只走横竖与 45 度，几条链共用一段路时平行排开，站外色环是今天的状态（红＝已停，橙＝收窄，紫＝受损，绿＝在用），虚线是绕行，点线是经过已停站的旧路；视野外的站贴在地图边缘写「往某地」',
