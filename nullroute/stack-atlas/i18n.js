...
const UI = {
  ...
  ov_sub: ['真实地图，记号的意思见图内左下角的图例；三个战区的供应链依赖关系是另一层地铁图，点地图左上角的按钮才显示。右侧是三个战区今天的读数，下面是今天的警报',
    'Real map; the legend at the map\'s bottom left explains each symbol. The supply-chain dependencies of the three theaters are a separate metro-map layer, shown only when you press the button at the top left of the map. Right: today\'s readings for the three theaters; below: today\'s alerts'],
  ...
  alerts_sub: ['{n} 条：断点、指标偏离、火点异常、影像疑似受损', '{n}: breaks, indicator deviations, fire anomalies, suspected damage in imagery'],
  ...
  lg_node: ['关键节点', 'Key node'], lg_node_note: ['带编号的定位针，按节点状态上色；点开看它的证据与上下游', 'numbered pin, coloured by the node\'s status; click for its evidence and links'],
  lg_node_colors: ['红＝中断，橙＝减少，紫＝受损，绿＝正常', 'red = cut off, amber = reduced, purple = damaged, green = normal'], lg_node_colors_note: ['中断与受损的针带呼吸的光晕；状态是人工判定，判定日见节点标签', 'cut-off and damaged pins carry a pulsing halo; status is a manual judgement, date on the node label'],
  ...
  ov_lg_hint: ['点一个记号看详情；点战区框进入那个战区；右上角图层钮开关每一层', 'Click a symbol for details; click a theater box to open that theater; the layers button at top right switches layers'],
  ov_lg_ck: ['咽喉点（近七日通过量对基线）', 'Chokepoints (last 7 days against baseline)'], ov_lg_ck_stopped: ['不到基线一半', 'under half of baseline'], ov_lg_ck_narrowed: ['一半到八成五', 'half to 85%'], ov_lg_ck_inuse: ['八成五以上', 'above 85%'],
  ov_lg_ck_note: ['大的是 Stack Atlas 里有节点的咽喉点；数来自 IMF PortWatch，每天算', 'Larger = chokepoints with a Stack Atlas node; counts from IMF PortWatch, computed daily'],
  ...
  anom_note: ['逐船航迹滞后约四天，这一版是 {d}；与开战前 2026 年 1 至 2 月的常态比', 'Per-vessel tracks about four days behind; this edition is {d}; compared with the pre-war pattern of January–February 2026'],
  ...
  nodes_sub: ['按紧要程度排；点一个节点看它的证据与上下游', 'Ranked by urgency; click a node for its evidence and links'],
  tiles_h2: ['指标格', 'Indicators'], map_tiles_h2: ['地图与指标格', 'Map and indicators'],
  tiles_sub: ['近七日日均对基线的比值；红边低于 0.85', '7-day mean against baseline; red rim below 0.85'],
  aligned_h2: ['同一时间轴上的对齐图', 'Aligned charts on one time axis'],
  aligned_sub: ['四百天，红色虚线是断点', '400 days; the red dashed line is the break'],
  ba_h2: ['断点前后的影响表', 'Before/after the break'], ba_sub: ['物理量、绕行、价格、新闻量', 'Physical quantities, rerouting, prices, news volume'],
  ...
  conflict_h2: ['冲突层', 'Conflict layer'], conflict_sub: ['GDELT 最近二十四小时', 'GDELT, last 24 hours'],
  ...
  company_note: ['提到不等于受影响', 'A mention is not an impact'],
  ...
  strikes_sub: ['火点落在设施三公里内且超过该站基线的日子', 'Days with fire detections within 3 km of a facility above its own baseline'],
  ...
  s2_sub: ['哨兵二号，十米，二公里窗口；每站一个结论', 'Sentinel-2, 10 m, 2 km window; one conclusion per site'],
  ...
  chains_sub: ['经过这一战区必经点的 {n} 条链，按停摆窗口排序', '{n} chains passing this theater\'s chokepoints, sorted by outage window'],
  ...
  va_note: ['全球渔业观察，卫星加岸站船位，滞后约四天，数据到 {d}。货船一栏含油轮。基线是 {b0} 至 {b1} 的日均，近况是最后 7 天', 'Global Fishing Watch, satellite and coastal AIS, about four days behind, data to {d}. Cargo includes tankers. Baseline {b0} to {b1}, recent = last 7 days'],
  ...
  ais_note: ['岸站船位快照，时刻 {t}（世界时），共 {n} 艘，航行中 {m} 艘', 'Coastal-station snapshot at {t} UTC, {n} ships, {m} under way'],
